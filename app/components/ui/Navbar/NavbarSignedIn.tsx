...
export default function NavbarSignedIn({ initialProfile }: NavbarSignedInProps) {
  ...
  const pathname = usePathname(); // Get the current path to style the active link
  ...
  const links = [
    ...
    { key: "projects", name: "Projects", path: "/projects" }, // Corrected key to be unique
  ];
        ...
        {links.map((link) => (
          <Link
            ...
            className={`cursor-pointer transition-colors text-sm xl:text-base whitespace-nowrap ${
              pathname === link.path ? 'text-white font-semibold' : 'text-gray-400 hover:text-white'
            }`}
          >
            {link.name}
          </Link>
              ...
              <div className="flex flex-col items-center gap-3 py-4 px-4">
                {links.map((link, index) => (
                  <motion.div key={link.key} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05, duration: 0.3 }} className="w-full">
                    <button onClick={() => handleMobileNavClick(link.path)} className="cursor-pointer hover:text-gray-400 transition-colors text-base w-full text-center py-2 block">
                      {link.name}
                    </button>
                  </motion.div>
                ))}
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.3 }} className="flex flex-col items-center gap-3 mt-2 border-t border-neutral-700 pt-4 w-full">
                    {leagueImage && (
                        <button onClick={() => handleMobileNavClick("/leaderboard")} className="flex items-center gap-2 bg-neutral-700/50 border border-neutral-600/60 p-2.5 rounded-full text-sm hover:bg-neutral-700 transition-colors">
                    ...
                    <UserAvatar user={displayProfile} onDropdownItemClick={() => setIsMenuOpen(false)} />
                </motion.div>
              </div>
            </motion.div>
          </>
