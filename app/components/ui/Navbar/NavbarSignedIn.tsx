...
import { hydrateProfile } from "@/app/store/features/profile/profileSlice";
import { LeaderboardImagesData } from "@/lib/data/leaderboardImagesData";
import UserAvatar from "../Avatar";

// Define the props to accept the server-fetched data
...
export default function NavbarSignedIn({ initialProfile }: NavbarSignedInProps) {
  ...
  // Hydrate the store with server data on initial render
  useLayoutEffect(() => {
    dispatch(hydrateProfile(initialProfile));
  }, [initialProfile, dispatch]);

  // Select data from the now-hydrated store
  const { profile } = useSelector((state: RootState) => state.profile);

  // The isLoading state is now false on the initial render!
  ...
  return (
    <nav className="relative flex items-center justify-between bg-[#262626] text-white py-3 sm:py-4 px-4 sm:px-6 lg:px-8 w-full">
        {/* The rest of your JSX remains exactly the same. No changes needed below this line. */}
