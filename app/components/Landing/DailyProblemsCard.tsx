...
export const DailyProblemCard = ({ problem }: { problem: DailyProblem }) => {
  ...
  return (
    <div className=" p-6 rounded-2xl  flex flex-col justify-between shadow-lg">
      <div>
        <div className="mb-3">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {problem.topic.map((t, index) => (
            <span
              key={index}
              className="bg-neutral-800 text-white text-xs font-medium px-2 py-0.5 rounded-full"
            >
              {t}
            </span>
          ))}
      ...
      <div className="mt-4 pt-4 border-t  flex items-center justify-between gap-4">
        <div className="flex items-center gap-x-4 gap-y-1  text-xs flex-wrap">
          <div className="flex items-center gap-1.5" title="Experience Points">
            ...
            <span className="font-semibold text-">{problemDifficulty.xp} XP</span>
          </div>
          ...
          <div className="flex items-center gap-1.5" title="Maximum Time">
            ...
            <span className="font-semibold text-">{problem.maxTime}min</span>
          </div>
        </div>
        
        <Link href={`/problems/${problem.id}`}>
          <button className="bg-green-600 hover:bg-green-700 text-white cursor-pointer text- font-bold py-2 px-4 rounded-md transition-all duration-300 flex items-center justify-center gap-1.5 text-sm group">
            Solve
            <ArrowRight
              size={14}
...
export const AllProblemsSolvedCard = () => (
  <div className="bg-neutral-900/50 p-6 rounded-2xl border border-neutral-800 flex flex-col justify-center items-center text-center">
    ...
    <Link href="/problems" className="w-full">
      <button className="w-full bg-neutral-700 hover:bg-neutral-600 text- font-bold py-2 px-4 rounded-md transition-colors flex items-center justify-center gap-2 text-sm">
        <Code size={16} />
        Browse Problems
      </button>
    </Link>
