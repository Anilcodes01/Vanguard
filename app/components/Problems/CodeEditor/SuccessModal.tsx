...
export const SuccessModal = ({ xp, stars, onClose }: { xp: number, stars: number, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-green-500/30 rounded-2xl p-8 text-center w-full max-w-sm relative animate-in fade-in-0 zoom-in-95 duration-500">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center border-4 border-zinc-900">
        ...
        <button 
          onClick={onClose} 
          className="mt-8 w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-lg transition-all"
        >
          Continue
        </button>
      </div>
